...
const OfferList = ({
  ...
  return (
    <div className="mb-3 flex  items-center">
      <span className="mr-3 flex h-[18px] w-full max-w-[18px] items-center justify-center rounded-full bg-slate-800 bg-opacity-10 text-primary dark:bg-slate-200">
        {status === "active" ? checkIcon : crossIcon}
      </span>
      <p className="m-0 text-base font-medium text-body-color dark:text-slate-400">{text}</p>
    </div>
  );
};
